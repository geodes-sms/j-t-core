...
public class MainTest {
	...
	public static void main(String[] args) throws Exception {
		...
		Pattern assignTables_pre = new Pattern("assignTables_pre", "../res/tables/assignTables_pre.xmi",
				tables_ramified);
		Pattern assignTables_NAC = new Pattern("assignTables_NAC", "../res/tables/assignTables_NAC.xmi",
				tables_ramified);
		ArrayList<Pattern> assignTables_NACS = new ArrayList<>();
		assignTables_NACS.add(assignTables_NAC);
		Pattern assignTables_pos = new Pattern("assignTables_pos", "../res/tables/assignTables_pos.xmi",
				tables_ramified);
		...
		BasicRule assignTables = RuleFactory.createFRule("AssignTables", assignTables_pre, assignTables_NACS,
				assignTables_pos, true, JTCoreConstant.SIMPLEMATCH_ALGORITHM);

		// Execution
		Packet p = new Packet(tables);

		p = fireMimi.packetIn(p);
		if (!fireMimi.isSuccess())
			throw fireMimi.getException();

		assignTables.packetIn(p);
		if (!assignTables.isSuccess())
			throw assignTables.getException();
