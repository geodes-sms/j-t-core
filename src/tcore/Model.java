...
public class Model implements EcoreSerializable {
	...
	/**
	 * Object-to-node mapping Used to get the node associated to the EObject when
	 * adding edges
	 */
	protected HashMap<EObject, Node> nodesByObjectMapping;

	/**
	 * Node-to-object mapping Used to get the object associated to the node to build
	 * the match set in VF2
	 */
	...
	/**
	 * @return Object associated to specified node, or null if node is not part of
	 *         the mapping Used to build the match set in VF2
	 */
	...
	/**
	 * Serializer for Models into a file readable easily by C++. NOTE: Format
	 * follows this structure. <code>
	 * [Number of nodes]
	 * <br>
	...
	public String serialize() {
		...
		for (int i = 0; i < objects.size(); i++) {
			...
			s.append("N_").append(EcoreUtil.getURI(node)).append(" ").append(node.eClass().getName()).append(" ")
					.append(attributeEList.size()).append("\n"); // N[node_id] [class] [number of attributes]
				...
				if (links instanceof EList) {
					...
					for (Object o : linksList) {
						if (o instanceof EObject) {
							linksSerialized.append(EcoreUtil.getURI((EObject) o)).append(" ").append(ref.getName())
									.append("\n");
						}
					}
				} else if (links instanceof EObject) {
					linkCounter++;
					linksSerialized.append(EcoreUtil.getURI((EObject) node.eGet(ref))).append(" ").append(ref.getName())
							.append("\n");
				} else {
					// throw new Exception();
				}
	...
	public HashMap<String, HashMap<String, Object>> getAttributes(EObject object, String className) {
		...
		// If the given object is not the motherclass
			...
			for (EObject object2 : resource.getContents().get(0).eContents()) {
				...
				// if the given object is a subclass of a component, we get its superclass
	 ...
	 * of objects - 1) label = value of identifier attribute className = name of
	 * class representing the object Every relation is represented as an edge with:
	 * source node = source object of relation target node = target object of
	 ...
	 * 
	 * <p>
	 * Every relation is represented as an edge with: source node = source object of
	 * relation target node = target object of relation label = label of source
	 * object -> label of target object
	 * </p>
	 */
	protected void addEdgesToGraph() {
		// Iterating through the nodes again once all of them are created to avoid the
		// situation that the target node does not exist in the graph when iterating
		// through the EReferences to attempt to add the edge
		for (Node gn : graph.nodes) {
			...
			for (EReference ref : referenceEList) {
				...
				// The link can either be an EList (it has more references under it) or an
				// EObject (it is the only linked object)
				if (links instanceof EList) {
					@SuppressWarnings("rawtypes")
					EList linksList = (EList) links;
					for (Object o : linksList) {
