 ...
 * This class is also responsible for computing and executing the CRUD
 * operations necessary for the rewriting process of a rule.
 *
...
public class RHS {
	...
	public void execute(Match matchToRewrite) throws Exception {
		...
		for (String createLabel : createLabels) {
			...
			if (modelObjectParent == null)
				throw new Exception(); // FIXME: 2017-12-08 Replace with better suited exception.
	...
	private void createNode(EObject modelObjectParent, EObject patternObject, EReference containmentFeature,
			Match match) {
		EStructuralFeature label = patternObject.eClass().getEStructuralFeature(Utils.MT_LABEL);
		...
		EStructuralFeature originalContainment = modelObjectParent.eClass()
				.getEStructuralFeature(originalContainmentName);
	...
	private void updateNode(EObject modelObject, EObject patternObject, Match match) {
		...
		for (EAttribute a : attributes) {
			if (a.getName().startsWith(Utils.MT_))
				continue;
			...
			String value = (modelObject.eGet(originalAttribute) == null) ? ""
					: modelObject.eGet(originalAttribute).toString();
		...
		for (EReference reference : references) {
			...
			for (EObject pc : list) {
				...
				EStructuralFeature originalReference = modelObject.eClass()
						.getEStructuralFeature(Utils.getOriginalName(reference.getName(), Utils.POST_));
