...
public class SimpleMatch extends RulePrimitive implements IMatchAlgo {
	...
	/**
	 * Tries to match the {@link LHS} to the {@link Model}. FIXME: 2017-12-08 The
	 * algorithm finds all the matches and filters out the submatches afterwards.
	...
	/**
	 * For a given submatch, recursively finds the matches that contain said
	 * submatch
	 *
	 ...
	 * @param match         The actual match containing the current label mappings
	 *                      so far
	...
	private ArrayList<Match> refactorMatches(Match match, ArrayList<Match> currentMatches, EObject child,
			String nextLabel, Object e) {
		if (e instanceof EObject && !match.getLabelMappings().containsValue(e) && objectEquals((EObject) e, child)) {
			...
			for (Match m : currentMatches) {
				...
				if (tmpMatch.getLabelMappings().containsValue(e))
					continue;
	...
	public boolean objectEquals(@NotNull EObject mObject, @NotNull EObject pObject) {
		...
		if (pObject.eGet(matchSubtypes).toString().equals("true")) {
			...
			if (!mClass.equals(pClass)) {
				if (subclasses == null || !subclasses.contains(Utils.PRE_ + mClass))
					return false;
			}
		...
		try {
			for (EAttribute ram_constraint : pObject.eClass().getEAllAttributes()) {
				...
				if (attributeName.startsWith(Utils.MT_))
					continue;
				...
				// TODO: 2017-12-08 Add support for other functions. Rewrite the RegEx to be
				// more inclusive.
				// Evaluate constraints with JS engine
				Object value = mObject.eGet(originalAttribute);
				String script = constraint.toString().replaceAll("getAttr\\(\\)",
